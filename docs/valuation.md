...
Estado atual: as cinco divergências de quantidade foram resolvidas por decisões auditadas. A brapi atualiza os ativos configurados para esse mecanismo; ativos NENHUM podem receber cotações e avaliações manuais com fonte e data. A normalização das moedas está concluída. A [reconciliação final](final-reconciliation.md) conferiu os valores com cobertura. Desde a migração `0053_historical_prices_in_valuation.sql`, a avaliação por data de corte também seleciona o último fechamento aceito em `market.daily_close_series`; com isso, o INRD11 usa R$ 72,65 em 27/08/2026 e não aparece mais como bloqueio no corte de 31/08/2026. A Brasilprev CICLO DE VIDA 2030 I PGBL continua sem avaliação documental nos escopos em que é incluída. A aceitação final permanece pendente. Não há conversão cambial implementada.
