...
class ApplyRetentionTests(unittest.TestCase):
    def setUp(self):
        self.fixture=test_retention_plan.RetentionTests();self.fixture.setUp()
    ...
    def test_only_verified_candidates(self):
        ...
        f=self.fixture
        expected={f.regular[0],f.regular[1],f.pre_updates[0],f.pre_updates[1],f.old_release}
        self.assertEqual(set(self.removed),expected)

    def test_stale_approval_removes_nothing(self):
        with self.assertRaises(ValueError):self.execute(self.current[:-1])
        self.assertEqual(self.removed,[])

    def test_missing_git_commit_removes_nothing(self):
        with self.assertRaises(ValueError):self.execute(self.current,lambda c:False)
        self.assertEqual(self.removed,[])

    def test_active_release_cannot_be_removed(self):
        for row in self.current:
            if row['path']==str(self.fixture.active):row['action']='candidate'
        with self.assertRaises(ValueError):self.execute(self.current)
        self.assertEqual(self.removed,[])
